...
function [dec_vec] = decimate_vector(given_vec, new_size)
	given_vec_size = max(size(given_vec));
	
	// Assuming row_vector; and assuming given_vec_size > new_size
	no_of_removal = given_vec_size - new_size;
	if no_of_removal <> 0 then
		step_size = given_vec_size / no_of_removal;
	
		// To fill in zeros, later 1 will be subtracted
		given_vec = given_vec+1;

		count = step_size;
		for i = 1:step_size:given_vec_size
			given_vec(i)=0;
		end
	
		// Remove Zeros from given_vec, shrinking,
		dec_vec = [];
		for i = 1 : given_vec_size
